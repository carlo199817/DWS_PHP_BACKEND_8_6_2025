...
class formula_change{
     ...
     public function setChangeformula($entityManager,$field,$collect_data) {
            ...
            $field = $entityManager->find(configuration_process\field::class,$field);
             ...
             if($link->getFieldtype()->getDescription()==="MATH"){
                    $selectanswer = json_decode($link->getFormula(), true);
        ...
        foreach($selectanswer[0]['choices'] as $index => $divisor){
            ...
            $link->setFormula(json_encode($selectanswer));
            $entityManager->flush();
        }
        }
                        }else if($link->getFieldtype()->getDescription()==="GRADE"){
                            $selectanswer = json_decode($link->getFormula(), true);
        ...
        foreach ($collect_data as $item) {
            ...
            $entityManager->flush();
                            ...
                            }else  if($link->getFieldtype()->getDescription()==="CONDITION"){
                                $selectanswer = json_decode($link->getFormula(), true);
            ...
            $entityManager->flush();
              ...
              $this->setChangeformula($entityManager,$field,$collect_data);
             }
