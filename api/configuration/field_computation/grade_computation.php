...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    "attributes" => [
     ...
     $field = $entityManager->find(configuration_process\field::class,$input['field_id']);
              if($field){
                    $grade_select_answer = json_decode($field->getFormula(), true);
                    if($grade_select_answer['choices'][0]['value']!=""){
                    $result = 0;
                    $operator = "";

                        $count = count($grade_select_answer['choices']);
                        for ($i = 0; $i < $count; $i++) {
                           if (isset($grade_select_answer['choices'][$i]['value'])) {
                            preg_match('/([\d.]+)\s*([<>])\s*([\d.]+)/', $grade_select_answer['choices'][$i]['value'], $matches);
                             if (count($matches) != 4) {
...
try {
    if ($value1<$value2){
        ...
        if($grade_select_answer['use_declare_value']){
             $decision  = [
                         'value'=>$grade_select_answer['decision'][$i]['value'],
                         'style'=>$grade_select_answer['decision'][$i]['style']
                          ];
        }else{
             $decision  = [
                         'value'=>$value1,
